...
pub fn get_file(original_file_hash: ActionHash) -> ExternResult<Option<Record>> {
    let links = get_links(original_file_hash.clone(), LinkTypes::FileUpdates, None)?;
    let latest_link = links
        .into_iter()
        .max_by(|link_a, link_b| link_b.timestamp.cmp(&link_a.timestamp));
    let latest_file_hash = match latest_link {
        Some(link) => AnyDhtHash::from(
            link.target.into_action_hash()
                .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Failed to convert link target to AnyDhtHash"))))?
        ),
